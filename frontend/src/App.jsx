...
function WorkspacePage({ page, children }) {
  return <motion.div className="route-frame" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: .25, ease: [.22, 1, .36, 1] }}>
    <div className="route-head"><div><div className="route-kicker section-label">{page.kicker}</div><h1 className="route-title">{page.title}</h1><p className="route-description">{page.description}</p></div><div className="route-index">{page.id === 'predictor' ? 'LIVE / 01' : page.id === 'historical' ? 'DATA / 02' : page.id === 'replay' ? 'TIME / 03' : 'FILE / 04'}</div></div>
