...
function MeshPreview() {
  ...
  return <div className="mesh-visual" aria-label="Live sensor mesh preview"><div className="mesh-orbit orbit-one" /><div className="mesh-orbit orbit-two" /><svg viewBox="0 0 100 100" role="img" aria-hidden="true"><defs><linearGradient id="meshLine" x1="0" x2="1"><stop offset="0" stopColor="#b7ff4a" stopOpacity=".18" /><stop offset="1" stopColor="#46e7f2" stopOpacity=".6" /></linearGradient><filter id="nodeGlow"><feGaussianBlur stdDeviation="1.8" result="blur" /><feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge></filter></defs>{links.map(([a,b]) => <line key={`${a}-${b}`} x1={points[a][0]} y1={points[a][1]} x2={points[b][0]} y2={points[b][1]} stroke="url(#meshLine)" strokeWidth=".45" />)}<circle cx="51" cy="53" r="11" fill="rgba(183,255,74,.08)" stroke="#b7ff4a" strokeOpacity=".55" strokeWidth=".3" /><path d="M51 44v18M42 53h18" stroke="#b7ff4a" strokeWidth=".45" strokeOpacity=".75" />{points.map(([x,y],i) => <g key={`${x}-${y}`} filter="url(#nodeGlow)"><circle cx={x} cy={y} r={i%4!==2?1.8:1.4} fill={i%4!==2?'#b7ff4a':'#ffab5c'} /><circle cx={x} cy={y} r="3.3" fill="none" stroke={i%4!==2?'#b7ff4a':'#ffab5c'} strokeOpacity=".22" strokeWidth=".5" /></g>)}</svg><div className="mesh-label mesh-label-top"><span className="status-dot" /> LIVE NODE GRID</div><div className="mesh-label mesh-label-bottom">MINE PANEL A / 07:42:18 IST</div><div className="mesh-readout"><span>RSSI</span><strong>-62 dBm</strong><i /><span>PACKETS</span><strong>99.98%</strong></div></div>;
}
...
export default function App() {
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${mobileNav ? 'sidebar-open' : ''}`}><div className="brand-lockup"><div className="brand-mark"><span /><span /><span /></div><div><strong>carbonex</strong><small>ground intelligence</small></div></div><button className="mobile-close" onClick={() => setMobileNav(false)} aria-label="Close navigation"><X size={20} /></button><div className="sidebar-kicker">Workspace</div><nav className="side-nav">{navItems.map(({id,label,icon:Icon}) => <button key={id} className={activeTab===id?'nav-item active':'nav-item'} onClick={() => {setActiveTab(id);setMobileNav(false);}}><Icon size={17}/><span>{label}</span>{activeTab===id&&<ChevronRight size={15} className="nav-arrow"/>}</button>)}</nav><div className="sidebar-spacer"/><div className="system-card"><div className="system-card-header"><span className="status-dot"/> SYSTEM NOMINAL</div><div className="system-card-value">99.98<span>%</span></div><div className="system-card-note">mesh availability / last 24h</div><div className="mini-bar"><i/></div></div><div className="sidebar-footer"><span>CARBONEX / 0.9.4</span><span className="env-pill">EDGE + CLOUD</span></div></aside>
    {mobileNav&&<button className="nav-scrim" aria-label="Close navigation" onClick={() => setMobileNav(false)}/>}<main className="main-content"><header className="topbar"><button className="mobile-menu" onClick={() => setMobileNav(true)} aria-label="Open navigation"><Menu size={22}/></button><div className="breadcrumb"><span>CARBONEX</span><ChevronRight size={14}/><strong>LIVE MONITOR</strong></div><div className="topbar-actions"><span className="sync-status"><span className="status-dot"/> synced 12s ago</span><button className="icon-button" title="Download report"><Download size={17}/></button><div className="avatar">AK</div></div></header>
      <section className="hero-section"><div className="hero-copy"><div className="eyebrow"><span className="eyebrow-line"/> MINE PANEL A / RANIGANJ COALFIELD</div><h1>See the ground<br/><em>before it moves.</em></h1><p className="hero-description">Carbonex turns distributed sensing into an early-warning advantage. From the first millidegree of tilt to a field-ready alert.</p><div className="hero-actions"><button className="primary-button" onClick={() => document.getElementById('workspace')?.scrollIntoView({behavior:'smooth'})}>Open live workspace <ArrowUpRight size={16}/></button><button className="text-button" onClick={() => setActiveTab('historical')}>Explore analytics <span>↗</span></button></div><div className="hero-proof"><div><strong>24 / 7</strong><span>continuous sensing</span></div><div><strong>4.2 km</strong><span>mesh coverage</span></div><div><strong>6 min</strong><span>early signal window</span></div></div></div><MeshPreview/></section>
      ...
      <section className="workspace-heading" id="workspace"><div><div className="section-label">OPERATOR CONSOLE</div><h2>Live field picture <span className="live-badge"><span className="status-dot"/> LIVE</span></h2></div><div className="workspace-meta"><span><Layers3 size={15}/> {liveNodes} nodes reporting</span><span><Gauge size={15}/> peak risk {activeRisk.toFixed(1)}</span></div></section><KpiCardGrid nodesSummary={nodesSummary} activeNode={selectedNode}/><MineNodeGrid nodesSummary={nodesSummary} selectedNode={selectedNode} onSelectNode={setSelectedNode}/><section className="module-wrap">{activeTab==='predictor'&&<RealTimePredictor activeNode={selectedNode}/>} {activeTab==='historical'&&<HistoricalAnalytics activeNode={selectedNode}/>} {activeTab==='replay'&&<LiveReplaySimulator activeNode={selectedNode}/>} {activeTab==='batch'&&<BatchCsvScorer/>}</section><footer className="app-footer"><div><strong>carbonex</strong> / ground intelligence platform</div><div>Built for safer extraction <span>•</span> Smart India Hackathon 2025</div></footer>
    </main></div>;
}
