...
function buildAtlas(tile, glyphFrac, color) {
...
export default function AsciiRadar({
  ...
  useEffect(() => {
    ...
    const buildField = () => {
      const patch = Math.max(2, (live.clusterSize | 0));
      const z =
        t *
        (Math.max(0, Math.min(100, live.clusterSpeed)) / 50) *
        CLUSTER_DRIFT_AT_50;
      const T = 0.5 - (Math.max(0, Math.min(100, live.density)) / 100 - 0.5) * 0.62;
    ...
    const render = (now) => {
      ...
      const nCols = Math.max(8, Math.min(400, (live.columns | 0)));
      ...
      if (key !== atlasKey) {
        ...
        atlas = buildAtlas(
          tile,
          Math.max(0.2, Math.min(1, (live.glyphSize | 0) / 100)),
          (live.glyphColor) || "#E5FF32"
        );
      }
      ...
      const half = Math.max(0.5, (live.ringWidth | 0) / 2);
      const v = (Math.max(0, Math.min(100, live.pingSpeed)) / 50) * PING_SPEED_AT_50;
      ringA.fill(0);
      fillA.fill(0);
      ...
      ctx.fillStyle = live.ringColor || "#E5FF32";
      for (let r = 0; r < rows; r++) {
